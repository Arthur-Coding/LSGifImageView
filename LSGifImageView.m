...
//  LSGifImageView.h
//
//  Created by ArthurShuai on 16/3/19.
//  Copyright © 2016年 ArthurShuai. All rights reserved.
//

#import "LSGifImageView.h"
#import <ImageIO/ImageIO.h>

@interface LSGifImageView ()
...
- (void)playGifAnimation {
    ...
    __weak LSGifImageView *weakSelf = self;
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        while (weakSelf.isPlay && weakSelf.lastCount > 0) {
            ...
            if ((weakSelf.isSourceChange || src == nil) && weakSelf.gifData != nil) {
                ...
                if (src) {
                    ...
                    frameDelayArray = [LSGifImageView delayArrayWithSource:src];
            ...
            if (image == nil) {
                image = [LSGifImageView imageWithSource:src andIndex:weakSelf.frameIndex];
        ...
        weakSelf.isPlay = NO;
        if (_playEndOperation) _playEndOperation();//播放结束之后的回调
    });
}
...
+ (NSArray<NSNumber*> *)delayArrayWithSource:(CGImageSourceRef)src {
    ...
    for (size_t i = 0; i < frameCount; i++) {
        NSTimeInterval delay = [LSGifImageView delayTimeWithSource:src andIndex:i];
...
+ (NSTimeInterval)delayTimeWithGifData:(NSData *)gifData andIndex:(size_t)index {
    ...
    if (src) {
        NSTimeInterval delay = [LSGifImageView delayTimeWithSource:src andIndex:index];
        CFRelease(src);
        return delay;
    }
    else {
...
+ (NSArray<NSNumber*> *)delayArrayWithGifData:(NSData *)gifData {
    CGImageSourceRef src = CGImageSourceCreateWithData((__bridge CFDataRef)gifData, NULL);
    if (src) {
        NSArray *array = [LSGifImageView delayArrayWithSource:src];
        CFRelease(src);
        return array;
    }
    else {
...
+ (UIImage *)imageWithGifData:(NSData *)gifData andIndex:(size_t)index {
    CGImageSourceRef src = CGImageSourceCreateWithData((__bridge CFDataRef)gifData, NULL);
    if (src) {
        UIImage *image = [LSGifImageView imageWithSource:src andIndex:index];
        CFRelease(src);
        return image;
    }
    else {
...
+ (NSArray<UIImage*> *)imageArrayWithGifData:(NSData *)gifData {
    ...
    if (src) {
        ...
        size_t frameCount = CGImageSourceGetCount(src);
        for (size_t i = 0; i < frameCount; i++) {
            UIImage *image = [LSGifImageView imageWithSource:src andIndex:i];
